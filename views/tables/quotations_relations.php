...
$aColumns = hooks()->apply_filters('quotations_relation_table_sql_columns', $aColumns);
...
foreach ($rResult as $aRow) {
    ...
    $numberOutput = '<a href="' . admin_url('quotations/list_quotations/' . $aRow['id']) . '">' . format_quotation_number($aRow['id']) . '</a>';

    $numberOutput .= '<div class="row-options">';

    $numberOutput .= '<a href="' . site_url('quotation/' . $aRow['id'] . '/' . $aRow['hash']) . '" target="_blank">' . _l('view') . '</a>';
    if (has_permission('quotations', '', 'edit')) {
        $numberOutput .= ' | <a href="' . admin_url('quotations/quotation/' . $aRow['id']) . '">' . _l('edit') . '</a>';
    }
    $numberOutput .= '</div>';

    $row[] = $numberOutput;

    $row[] = '<a href="' . admin_url('quotations/list_quotations/' . $aRow['id']) . '">' . $aRow['subject'] . '</a>';
