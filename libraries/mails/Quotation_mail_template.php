...
class Quotation_mail_template
{
    ...
    private function get_language($email, $template = null, $params = [])
    {
        ...
        if ($rel_type != 'quotation' && $rel_type != 'lead') {
        ...
        } elseif ($rel_type == 'quotation') {
            $this->ci->db->select('rel_type, rel_id');
            $this->ci->db->where('id', $this->get_rel_id());
            $quotation = $this->ci->db->get(db_prefix() . 'quotations')->row();
            if ($quotation && $quotation->rel_type == 'lead') {
                $this->ci->db->select('default_language')
                ->where('id', $quotation->rel_id);

                $lead = $this->ci->db->get(db_prefix() . 'leads')->row();
            } elseif ($quotation && $quotation->rel_type == 'customer') {
                $customerDefault = get_client_default_language($quotation->rel_id);
    ...
    {
        include_once(quotation_get_mail_template_path($className, $params));
